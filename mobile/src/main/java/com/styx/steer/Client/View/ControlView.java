...
import com.styx.steer.Client.Activity.ControlActivity;
...
import com.styx.steer.Protocol.action.ScreenCaptureRequestAction;


@SuppressLint({
        "InlinedApi", "ClickableViewAccessibility"
})
public class ControlView extends ImageView
{
	...
	public ControlView(Context context, AttributeSet attrs)
	{
		...
		this.application = (Steer) this.controlActivity.getApplication();
	...
	protected synchronized void onWindowVisibilityChanged(int visibility)
	{
		...
		if (visibility == VISIBLE)
		{
			...
			this.controlActivity.findViewById(R.id.inputLayout).setVisibility(this.useEditText ? android.view.View.VISIBLE : android.view.View.GONE);
			// Set orientation
			if (debugging)
	...
	public synchronized void receiveAction(final com.styx.steer.Protocol.action.ScreenCaptureResponseAction action)
	{
	...
	private void reloadPreferences()
	{
		...
		//	this.screenCaptureEnabled = this.preferences.getBoolean("screenCapture_enabled", true);
		...
		this.screenCaptureCursorEnabled = this.preferences.getBoolean("screenCapture_cursor_enabled", true);
